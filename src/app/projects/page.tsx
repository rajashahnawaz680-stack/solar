...
export const metadata = {
  title: 'Our Solar Projects | SUNORA ENERGY',
    ...
    <main className="min-h-screen bg-slate-50 pt-28 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-extrabold text-[#0B0F0E] mb-6 tracking-tight">Our Solar Projects</h1>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {['All', 'Residential', 'Commercial', 'Rooftop', 'Hybrid', 'On-Grid'].map(cat => (
             <span key={cat} className={`px-5 py-2 rounded-full text-sm font-bold cursor-pointer transition-colors ${cat === 'All' ? 'bg-[#0B0F0E] text-white shadow-md' : 'bg-white border border-slate-200 text-slate-600 hover:border-[#B8D83D] hover:text-[#9dbf2c]'}`}>
                {cat}
             </span>
          ))}
        </div>
          ...
          {projectsData.map((project) => (
            <Link href={`/projects/${project.id}`} key={project.id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-200 flex flex-col transform hover:-translate-y-1 block">
              <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-200">
                ...
                <div className="absolute inset-0 bg-[#0B0F0E]/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <span className="bg-white/95 text-[#0B0F0E] font-bold px-6 py-2 rounded-full shadow-lg text-sm flex items-center">
                    <svg className="w-4 h-4 mr-2 text-[#9dbf2c]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    View Project
                  </span>
                </div>
                <div className="absolute top-4 left-4 bg-[#B8D83D] text-[#0B0F0E] text-xs font-bold px-3 py-1 rounded-full shadow-sm">
                  {project.category}
                </div>
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="text-xl font-bold text-[#0B0F0E] mb-2">{project.title}</h3>
