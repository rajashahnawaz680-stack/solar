    ...
    return (
      <main className="min-h-screen bg-slate-50 pt-28 pb-24 flex items-center justify-center">
        <div className="max-w-md w-full mx-auto p-8 bg-white rounded-3xl shadow-2xl text-center border border-slate-100">
           ...
           <Link href="/" className="block w-full bg-slate-900 text-white font-bold py-4 rounded-xl hover:bg-slate-800 transition-colors mb-4">
             Return Home
           </Link>
    ...
    <main className="min-h-screen bg-slate-50 pt-28 pb-24">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-4 tracking-tight">Get Your Free Solar Quote</h1>
        ...
        <div className="mb-10 flex justify-between relative">
           ...
           <div className="absolute top-1/2 left-0 h-1 bg-amber-400 -z-10 -translate-y-1/2 transition-all duration-300" style={{ width: `${((step - 1) / 5) * 100}%` }}></div>
           {[1,2,3,4,5,6].map(s => (
             <div key={s} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${s <= step ? 'bg-amber-400 text-slate-900' : 'bg-slate-200 text-slate-400'}`}>
               {s}
             </div>
           ))}
        </div>
           ...
           <form onSubmit={submit}>
             ...
             {step === 1 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">1. Personal Information</h3>
                 <div>
             ...
             {step === 2 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">2. Location & Property</h3>
                 <div>
             ...
             {step === 3 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">3. Energy Consumption</h3>
                 <div>
             ...
             {step === 4 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">4. System Requirements</h3>
                 <div>
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {['Not Sure', '3kW', '5kW', '10kW', '15kW', 'Custom'].map(opt => (
                         <div key={opt} onClick={() => setFormData({...formData, system: opt})} className={`p-4 rounded-xl text-center cursor-pointer border-2 transition-all ${formData.system === opt ? 'border-amber-400 bg-amber-50 text-amber-700 font-bold shadow-md' : 'border-slate-200 hover:border-amber-200 text-slate-600'}`}>
                           {opt}
                         </div>
                      ))}
                   </div>
             ...
             {step === 5 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">5. Payment Preference</h3>
                 <div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {['Cash / Upfront', 'Installments', 'Not Sure'].map(opt => (
                         <div key={opt} onClick={() => setFormData({...formData, payment: opt})} className={`p-4 rounded-xl text-center cursor-pointer border-2 transition-all ${formData.payment === opt ? 'border-amber-400 bg-amber-50 text-amber-700 font-bold shadow-md' : 'border-slate-200 hover:border-amber-200 text-slate-600'}`}>
                           {opt}
                         </div>
                      ))}
                   </div>
             ...
             {step === 6 && (
               <div className="space-y-6 animate-fade-in">
                 <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">6. Additional Information</h3>
                 <div>
                 ...
                 <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-start mt-6">
                    ...
                    <p className="text-xs text-slate-500">Your information is strictly confidential. By clicking Request My Solar Quote, you allow Solar KCKY to contact you regarding your solar requirements.</p>
                 </div>
               </div>
             )}
             ...
             <div className="flex justify-between mt-10 pt-6 border-t border-slate-100">
               ...
               {step < 6 ? (
                 <button type="button" onClick={nextStep} className="bg-slate-900 text-white font-bold px-8 py-3 rounded-xl hover:bg-slate-800 transition-colors shadow-md">
                   Next Step
                 </button>
               ) : (
                 <button type="submit" className="bg-amber-400 text-slate-900 font-extrabold px-8 py-3 rounded-xl hover:bg-amber-500 transition-colors shadow-lg">
